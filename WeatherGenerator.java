...
public class WeatherGenerator {
     ...
     * there exist drywet.txt and wetwet.txt that contains probabilities of the next
     * day being wet
     ...
     * {-97.58, 26.02, 0.76, 0.75, 0.77, 0.74, 0.80, 0.86, 0.94, 0.97, 0.89, 0.77,
     * 0.74, 0.77}
     * you can assume that there more than enough data in the txt file,
     * when there are more data in the txt files than what drywet & wetwet can
     * store, store it up to the array size
     * Return:
     * this method does not return data, the method is used to populate two 2D
     * arrays with 14
    ...
    /*
     * Description:
     * this method uses drywet and wetwet arrays populated by populateArrays, and
     * longitude and latitude
     * of the target location to populate drywetProbability and wetwetProbability
     * with the
     * probability of dry/wet day is followed by a wet day each month at that
     * location.
     * In other words, extracting the probabilities of the location.
     * parameters:
     * drywetProbability: array of size 12 that will be populated with by the
     * probability of dry days
     * followed by a wet day each month
     * wetwetProbability: array of size 12 that will be populated with by the
     ...
     * drywet:
     * a 2D array of doubles populated by the method populateArrays() using
     * drywet.txt
     * wetwet:
     * a 2D array of doubles populated by the method populateArrays() using
     * wetwet.txt
     * return:
     * this method does not return data. The method is used to populate two 1D
     * arrays of length
     * 12 - drywetProbability and wetwetProbability.
     * The probability of January is stored at index 0, February stored at index
     * 1...
     * example:
     ...
     * populateLocationProbabilities(drywetProbability, wetwetProbability, -97.58,
     * 26.02, drywet, wetwet);
    ...
    public static void populateLocationProbabilities(double[] drywetProbability, double[] wetwetProbability,
        ...
        for (int i = 0; i < wetwet.length; i++) {
            if (wetwet[i][0] == longitude && wetwet[i][1] == latitude) {
                for (int j = 2, count = 0; j < 14; j++, count++) {
                    wetwetProbability[count] = wetwet[i][j];
                }
            }
        }
        for (int i = 0; i < drywet.length; i++) {
            if (drywet[i][0] == longitude && drywet[i][1] == latitude) {
                for (int j = 2, count = 0; j < 14; j++, count++) {
                    drywetProbability[count] = drywet[i][j];
                }
            }
        }
    }
    ...
    /*
     * Description:
     * Given the number of days in a month, and probabilities of weather changing at
     * a certain location,
     * the method should return the forecast for the month.
     * The first day of the month has a 50% chance to be a wet day, [0,0.5) (wet),
     * [0.5,1) (dry)
     * Use StdRandom.uniform() to generate a real number uniformly in [0,1)
     * parameters:
     * drywetProbability:
     * a double representing the probability of next day being a wet day when the
     * current day is a dry day.
     * wetwetProbability:
     * a double representing the probability of next day being a wet day when the
     * current day is a wet day.
     * numberOfDays:
     * an integer representing how many days are in this month.
     * return:
     * The return should be an integer array of 1 and 2s, with the size equal to the
     * size of the month
     * 1 meaning the day is a wet day, 2 being a dry day. Index 0 being the first
     * day of the month;
     * example:
     * int[] forecast = forecastGenerator( 0.27, 0.55, 31);
     * Here, forecast shoule be a size 31 array of 1s and 2s, although the
     * probability is determined, the
     * return result will still be different for each run since it is randomly
     * generated.
     */
    ...
    /*
     * Description:
     * This method takes the number of locations that is stored in wetwet.txt and
     * drywet.txt (the number of
     * lines in each file), and takes in the month number (January is index 0,
     * February is index 1... ),
     * and the longitude and the latitude of the location we want to make the
     * prediction on.
     * This method calls all previous methods (populateArrays(),
     ...
     * Secondly, use populateLocationProbabilities to get the probability data of
     * the target location
     * Lastly, pass in the probability data of the month into forecastGenerator to
     * generate a forecast
     * parameters:
     * numberOfLocations:
     * an interger representing how many lines exists in wetwet.txt and drywet.txt
     * month:
     * an integer representing the month of the prediction (January is index 0,
     * February is index 1...)
     * longitude:
     ...
     * return:
     * return should be an integer array of 1 and 2s, with the size equal to the
     * size of the month
     * 1 meaning the day is a wet day, 2 being a dry day. index 0 being the first
     * day of the month;
     * example:
     ...
     * the return result will be different for each run since it is randomly
     * generated.
     */
    ...
    /*
     * Description:
     * Given the forecast of a month at certain location, this method finds the
     * index of the
     * first day of a 7 day period with the least amount of rain. If multiple exist,
     * return
     * the earliest.
     * parameters:
     * forecast:
     ...
     * index 0 is the first day of the month;
     * return:
     * returns the index of the first day of the 7 days period with the most dry
     * days in forecast.
